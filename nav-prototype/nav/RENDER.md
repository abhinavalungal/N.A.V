...
**Weather shows "Mock data" on part of the track.**
Expected on long voyages: live forecasts reach about a week ahead, so the far
end of a four-week passage is modelled. The panel says what share was live.
If *everything* says mock, Open-Meteo could not be reached — check the service
logs. To turn live data off entirely, set `WEATHER_PROVIDER=mock`; note the
free tier is licensed for non-commercial use, so read the licence note in
`DEPLOY.md` before showing it to a client.
