      ...
      <div className="border-t border-hairline p-3">
          ...
          <div className="mb-3 flex flex-wrap gap-1.5">
            {prompts.map((prompt) => (
              <button
                key={prompt}
                onClick={() => send(prompt)}
                className="border border-hairline bg-deck px-2 py-1 text-2xs text-dim transition-colors hover:border-faint/60 hover:text-ink"
              >
                {prompt}
              </button>
            ))}
        ...
        >
          <input
            ...
            disabled={busy}
            className="h-9 flex-1 rounded-sm border border-hairline bg-deck px-2.5 text-xs text-ink placeholder:text-faint focus:border-faint"
          />
