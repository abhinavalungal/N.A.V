      ...
      {now ? (
        <div className="grid grid-cols-2 gap-4 p-4">
          ...
          <Cell
            ...
            sub={`from ${compass(now.wave_direction_deg)}`}
          />
          <Cell
            label="Current"
          ...
          />
          <Cell label="Visibility" value={`${num(now.visibility_nm, 1)} NM`} sub={`${num(now.temperature_c, 1)} °C`} />
        </div>
      ) : null}
      ...
      {track ? (
        <div className="space-y-3 border-t border-hairline p-4">
          ...
          <p className="text-2xs text-faint">
            {track.samples.length} sample points · source{" "}
  ...
  return (
    <div>
      <svg viewBox="0 0 100 32" preserveAspectRatio="none" className="h-12 w-full">
        <polyline points={`0,32 ${points} 100,32`} fill="rgba(76,155,196,0.16)" stroke="none" />
        <polyline points={points} fill="none" stroke="#4C9BC4" strokeWidth="0.8" vectorEffect="non-scaling-stroke" />
      </svg>
