  ...
  return {
    ...
    layers: [
      { id: "sea", type: "background", paint: { "background-color": SEA } },
      {
        ...
        type: "line",
        source: "graticule",
        paint: { "line-color": GRID, "line-width": 0.6 },
      },
  ...
  if (marker.kind === "vessel") {
    el.innerHTML = `
      <svg width="22" height="22" viewBox="0 0 22 22" style="transform: rotate(${marker.heading ?? 0}deg); overflow: visible;">
        <circle cx="11" cy="11" r="9.5" fill="rgba(227,165,75,0.14)" stroke="rgba(227,165,75,0.45)" />
        <path d="M11 3 L15.5 17 L11 14 L6.5 17 Z" fill="${marker.muted ? "#8CA3B8" : "#E3A54B"}" />
      </svg>`;
  } else if (marker.kind === "waypoint") {
    el.innerHTML = `<svg width="8" height="8" viewBox="0 0 8 8"><circle cx="4" cy="4" r="2.5" fill="#4C9BC4" opacity="0.8"/></svg>`;
  } else {
    const color = marker.kind === "origin" ? "#8CA3B8" : "#4FB286";
    el.innerHTML = `
        ...
        pin.setPopup(
          new maplibregl.Popup({ offset: 14, closeButton: false }).setHTML(
            `<div style="font-weight:500">${marker.label}</div>${
              marker.detail ? `<div style="color:#8CA3B8;margin-top:2px">${marker.detail}</div>` : ""
            }`,
          ),
        );
      }
