...
carries it). `render.yaml` now ships `WEATHER_PROVIDER=auto`, so a deployment
pulls live forecasts by default. Volume is not the problem — a whole route is
two requests, and results are cached — but the licence is about *who* is
using it. For a GeoServe demo shown to clients, either switch that variable
to `mock` or buy an Open-Meteo customer plan and point the provider at
