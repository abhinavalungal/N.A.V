...
**Dark or light.** The switch is at the right of the top bar. It follows the operating system
on a first visit and remembers the choice after that. The chart and the graphs repaint with
it, since both read the same palette variables.
...
Open-Meteo is free and needs no key, and `auto` (the default) uses it.

A whole route costs **two requests**, whatever the number of waypoints: the marine endpoint
for waves and surface current, the forecast endpoint for wind, visibility and temperature.
Both take comma-separated coordinate lists and a date range, and each waypoint then picks the
hour nearest the time the vessel is expected to be there — read off the API's own
`hourly.time` array, not assumed by index. Wind is requested in knots, and the forecast call
uses `cell_selection=sea` so a coastal waypoint is not snapped to a land cell. Results are
cached in memory per point-hour.

Live data reaches about a week ahead. A four-week voyage therefore has a live near track and a
modelled far end; `live_fraction` on the track response says how much of it was real, and the
weather panel shows that share.

Anything that fails — network down, rate limited, a grid cell with no marine data, a time past
the forecast horizon — degrades to the mock provider for the affected points only.
...
* **Passage plan** down the left — the voyage split into legs, each with the time the vessel
  is expected there, the distance, the speed to hold, the fuel that leg burns and the weather
  it was costed against. Click a leg to pick it out on the chart.
* **Chart** in the middle, with the plan, a wind field drawn as arrows coloured by strength,
  and heavy-weather stretches painted over the track in amber or red.
* **Time scrubber** under the chart. Drag or press play, and the vessel marker moves to where
  she is expected to be at that moment while the wind field reloads for that hour.
* **Conditions along the track** — wind, sea, current, speed and fuel per leg, in a strip you
  read left to right like a router's weather table.
* **Weather warnings** on the right: every stretch above 34 kn or 4 m, merged into windows
  with a start and end time. Click one and the chart jumps to it.
* **Compare route options** is one button, not a separate section of the app. Each option
  shows cost in dollars alongside fuel and CO2, and says "Good", "Moderate" or "Rough
  conditions" rather than a risk index.

Thresholds: gale force 34 kn or 4 m seas raises a watch; 48 kn or 6 m makes it dangerous.
Both live in `backend/app/services/voyage_plan.py`.
